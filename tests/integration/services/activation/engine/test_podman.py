...
def test_get_podman_client(settings):
    ...
    uid_501_mock = mock.Mock(return_value=501)
    ...
    with mock.patch("os.getuid", uid_501_mock):
        client = get_podman_client()
        assert (
            client.api.base_url.netloc
            == "%2Frun%2Fuser%2F501%2Fpodman%2Fpodman.sock"
        )
...
def test_set_auth_json(podman_engine):
    ...
    with mock.patch("os.path.dirname"):
        ...
        assert engine.auth_file == "/run/user/501/containers/auth.json"
