...
@pytest.mark.parametrize(
    ...
    [
        (ImagePullPolicy.ALWAYS, True, True),
        (ImagePullPolicy.ALWAYS, False, True),
        (ImagePullPolicy.NEVER, True, False),
        (ImagePullPolicy.NEVER, False, True),
        (ImagePullPolicy.IF_NOT_PRESENT, True, False),
        (ImagePullPolicy.IF_NOT_PRESENT, False, True),
    ],
)
@pytest.mark.django_db
...
def test_engine_never_pull_policy_image_not_found(
    ...
    Note: Current implementation still pulls when image doesn't exist,
    even with Never policy due to the OR condition in the pull logic.
    """
    engine = podman_engine
    request = get_request_with_pull_policy(
    ...
    # With current implementation, it will still pull even with Never policy
    engine.start(request, log_handler)
...
def test_engine_if_not_present_pull_policy_missing_image(
    ...
    engine = podman_engine
    request = get_request_with_pull_policy(
        init_podman_data, default_organization, ImagePullPolicy.IF_NOT_PRESENT
    )
    log_handler = DBLogger(init_podman_data.activation_instance.id)
...
def test_engine_pull_policy_enum_values_work(
    ...
    for policy in [
        ...
        if policy == ImagePullPolicy.ALWAYS:
            ...
            engine.client.images.pull.assert_not_called()
        elif policy == ImagePullPolicy.IF_NOT_PRESENT:
