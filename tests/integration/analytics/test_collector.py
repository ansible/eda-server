...
def test_gather_when_enabled(mock_collector_cls, mock_flag_enabled):
    """Test gather function when FEATURE_EDA_ANALYTICS_ENABLED is enabled"""
...
def test_gather_when_disabled(mock_flag_enabled):
    """Test gather function when FEATURE_EDA_ANALYTICS_ENABLED is disabled"""
    mock_flag_enabled.return_value = False
    mock_logger = MagicMock()

    result = gather(logger=mock_logger)

    mock_logger.info.assert_called_once_with("FEATURE_EDA_ANALYTICS_ENABLED is disabled.")
