...
@override_settings(FLAGS={"FEATURE_EDA_ANALYTICS_ENABLED": [("boolean", True)]})
...
def test_gather_analytics_command_by_ff_state(
    ...
    with mock.patch(
        ...
        logger = logging.getLogger("aap_eda.analytics")
        eda_log = caplog_factory(logger)
        with override_settings(FLAGS={"FEATURE_EDA_ANALYTICS_ENABLED": [("boolean", feature_flag_state)]}):
            command = "gather_analytics"
            call_command(command, stdout=out)
