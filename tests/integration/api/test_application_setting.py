...
@pytest.mark.parametrize(
    "feature_flag_state, status_code",
...
def test_partial_update_system_settings(
    superuser_client: APIClient, feature_flag_state, status_code
):
    with override_settings(
        FLAGS={"EDA_ANALYTICS": [("boolean", feature_flag_state)]}
...
@pytest.mark.parametrize(
    "feature_flag_state, status_code",
    [(True, status.HTTP_403_FORBIDDEN), (False, status.HTTP_404_NOT_FOUND)],
)
def test_list_settings_forbidden(
...
@pytest.mark.parametrize(
    "feature_flag_state, status_code",
    [(True, status.HTTP_400_BAD_REQUEST), (False, status.HTTP_404_NOT_FOUND)],
)
