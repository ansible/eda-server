...
def test_create_decision_environment(
    ...
    if status_code == status.HTTP_201_CREATED:
        ...
        pull_policy_enum = ImagePullPolicy.from_user_input(
            expected_result["pull_policy"]
        )
        expected_result["pull_policy"] = pull_policy_enum.to_display()
...
def assert_de_base_data(
    ...
    pull_policy_enum = ImagePullPolicy.from_user_input(expected.pull_policy)
    assert response["pull_policy"] == pull_policy_enum.to_display()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("IfNotPresent", status.HTTP_201_CREATED),
        ("always", status.HTTP_201_CREATED),  # Test lowercase frontend value
        ("never", status.HTTP_201_CREATED),  # Test lowercase frontend value
        ("missing", status.HTTP_201_CREATED),  # Test lowercase frontend value
...
def test_create_decision_environment_with_pull_policy(
    ...
    if expected_status == status.HTTP_201_CREATED:
        ...
        # Normalize the input value to what should be stored internally
        expected_internal_policy = (
            ImagePullPolicy.from_user_input(pull_policy)
            if pull_policy
            else ImagePullPolicy.ALWAYS
        )
        assert de.pull_policy == expected_internal_policy.value
        assert (
            response.data["pull_policy"]
            == expected_internal_policy.to_display()
        )
    else:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Never", "IfNotPresent", status.HTTP_200_OK),
        ("IfNotPresent", "Always", status.HTTP_200_OK),
        ("Always", "never", status.HTTP_200_OK),
...
def test_patch_decision_environment_pull_policy(
    ...
    if expected_status == status.HTTP_200_OK:
        de.refresh_from_db()
        # Normalize the update value to what should be stored internally
        expected_internal_policy = ImagePullPolicy.from_user_input(
            update_policy
        )
        assert de.pull_policy == expected_internal_policy.value
        assert (
