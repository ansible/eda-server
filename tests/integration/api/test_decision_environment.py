...
def test_create_decision_environment_with_pull_policy(
    ...
    if expected_status == status.HTTP_201_CREATED:
        ...
        expected_pull_policy = (
            pull_policy if pull_policy else ImagePullPolicy.ALWAYS
        )
        assert de.pull_policy == expected_pull_policy
        assert response.data["pull_policy"] == expected_pull_policy
    else:
        assert "is not a valid choice" in str(response.data["pull_policy"])
