...
def test_create_decision_environment(
    ...
    if status_code == status.HTTP_201_CREATED:
        ...
        expected_result["pull_policy"] = convert_pull_policy_to_frontend(
...
def assert_de_base_data(
    ...
    assert response["pull_policy"] == convert_pull_policy_to_frontend(
        expected.pull_policy
    )
...
def test_create_decision_environment_with_pull_policy(
    ...
    if expected_status == status.HTTP_201_CREATED:
        ...
        expected_internal_policy = (
            convert_pull_policy_from_frontend(pull_policy)
            if pull_policy
            else ImagePullPolicy.ALWAYS
        )
        assert de.pull_policy == expected_internal_policy
        assert response.data["pull_policy"] == convert_pull_policy_to_frontend(
            expected_internal_policy
        )
    else:
...
def test_patch_decision_environment_pull_policy(
    ...
    if expected_status == status.HTTP_200_OK:
        ...
        expected_internal_policy = convert_pull_policy_from_frontend(
            update_policy
        )
        assert de.pull_policy == expected_internal_policy
        assert response.data["pull_policy"] == convert_pull_policy_to_frontend(
