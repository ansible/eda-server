...
def test_run_rsyslog_configurer_success(mocker):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        config_dir = mocker.patch(
            "aap_eda.utils.external_logging.getattr", return_value=temp_dir
        )
        config_string = "$WorkDirectory /var/lib/eda/rsyslog"
        mocker.patch(
            "aap_eda.utils.external_logging.construct_rsyslog_conf_template",
            return_value=config_string,
        )
        supervisor_cmd = mocker.patch(
            "aap_eda.utils.external_logging.supervisor_service_command"
        )
...
def test_run_rsyslog_configurer_exception(mocker):
    expected_message = (
        "Unhandled Exception in reconfigure_rsyslog: Test Exception"
    )
    with pytest.raises(CommandError) as e:
        mocker.patch(
            "aap_eda.utils.external_logging.getattr",
