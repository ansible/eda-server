...
def get_external_secrets(credential_id: int) -> dict:
    ...
    for obj in models.CredentialInputSource.objects.filter(
        ...
        try:
            ...
            msg = (
                f"Error fetching {obj.input_field_name} "
                f"defined in {obj.source_credential.name} using the external "
                f"credentials defined in {obj.target_credential.name} "
                f"Error: {str(err)}"
            )
            LOGGER.error(msg)
            raise RuntimeErrorFetchingExternalSecret(msg) from err
    return result
