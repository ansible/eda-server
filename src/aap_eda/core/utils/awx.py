...
def validate_pem(data, min_keys=0, max_keys=None, min_certs=0, max_certs=None):
    """
    Validate the given PEM data.

    Return a list of PEM objects, where each object is a dict with the
    following keys:
      ...
      - 'key_enc': Only when type == 'PRIVATE KEY', boolean indicating if key
        is encrypted.
    """
    ...
    while data:
        ...
        if not match:
            raise ValidationError(
                _("Invalid certificate or key: %s...") % data[:100]
            )
        ...
        if pem_obj_type.endswith("PRIVATE KEY"):
            ...
            try:
                ...
                raise ValidationError(
                    _('Invalid private key: unsupported type "%s"') % key_type
                )
        elif pem_obj_type == "CERTIFICATE":
            cert_count += 1
        else:
            raise ValidationError(
                _('Unsupported PEM object type: "%s"') % pem_obj_type
            )
        ...
        if pem_obj_info.get("key_type", "") == "ed25519":
            # https://github.com/openssh/openssh-portable/blob/master/sshkey.c#L3218
            # Decoded key data starts with magic string (null-terminated),
            # four byte length field, followed by the ciphername --
            # if ciphername is anything other than 'none' the key is encrypted.
            pem_obj_info["key_enc"] = not bool(
                pem_obj_info["bin"].startswith(
                    b"openssh-key-v1\x00\x00\x00\x00\x04none"
                )
            )
    ...
    # Validate that the number of keys and certs provided are within
    # the limits.
    key_count_dict = {
        "min_keys": min_keys,
        "max_keys": max_keys,
    ...
    if key_count < min_keys:
        if min_keys == 1:
            if max_keys == min_keys:
                raise ValidationError(
                    _("Exactly one private key is required.")
                )
            else:
                raise ValidationError(
                    _("At least one private key is required.")
                )
        else:
            raise ValidationError(
                _(
                    "At least %(min_keys)d private keys are required, "
                    "only %(key_count)d provided."
                )
                % key_count_dict
            )
        ...
        if max_keys == 1:
            raise ValidationError(
                _("Only one private key is allowed, %(key_count)d provided.")
                % key_count_dict
            )
        else:
            raise ValidationError(
                _(
                    "No more than %(max_keys)d private keys are allowed, "
                    "%(key_count)d provided."
                )
                % key_count_dict
            )
    ...
    if cert_count < min_certs:
        if min_certs == 1:
            if max_certs == min_certs:
                raise ValidationError(
                    _("Exactly one certificate is required.")
                )
            else:
                raise ValidationError(
                    _("At least one certificate is required.")
                )
        else:
            raise ValidationError(
                _(
                    "At least %(min_certs)d certificates are required, "
                    "only %(cert_count)d provided."
                )
                % cert_count_dict
            )
        ...
        if max_certs == 1:
            raise ValidationError(
                _("Only one certificate is allowed, %(cert_count)d provided.")
                % cert_count_dict
            )
        else:
            raise ValidationError(
                _(
                    "No more than %(max_certs)d certificates are allowed, "
                    "%(cert_count)d provided."
                )
                % cert_count_dict
            )
...
def validate_certificate(data):
    """
    Validate that data contains one or more certificates.

    Adds BEGIN/END lines if necessary.
    """
...
def validate_ssh_private_key(data):
    ...
    Data contains at least one private key and optionally
