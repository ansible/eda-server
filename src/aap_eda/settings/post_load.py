...
def _set_resource_server(settings: Dynaconf) -> None:
    if not settings.RESOURCE_SERVER:
        settings.RESOURCE_SERVER = {
            "URL": settings.RESOURCE_SERVER__URL,
            "SECRET_KEY": settings.RESOURCE_SERVER__SECRET_KEY,
            "VALIDATE_HTTPS": settings.RESOURCE_SERVER__VALIDATE_HTTPS,
        }
    ...
    if (
        ...
        settings.RQ_PERIODIC_JOBS.append(
            {
...
def post_loading(settings: Dynaconf):
    ...
    settings.DEBUG = get_boolean(settings, "DEBUG")
    settings.ALLOWED_HOSTS = _get_list_from_str(settings, "ALLOWED_HOSTS")
    ...
    settings.RULEBOOK_WORKER_QUEUES = _get_list_from_str(
    ...
    if settings.EVENT_STREAM_BASE_URL:
        settings.EVENT_STREAM_BASE_URL = (
            settings.EVENT_STREAM_BASE_URL.strip("/") + "/"
        )

    if settings.EVENT_STREAM_MTLS_BASE_URL:
        settings.EVENT_STREAM_MTLS_BASE_URL = (
            settings.EVENT_STREAM_MTLS_BASE_URL.strip("/") + "/"
        )
