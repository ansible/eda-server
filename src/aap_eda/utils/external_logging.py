...
def construct_rsyslog_conf_template(settings=settings):
    ...
    spool_directory = getattr(settings, 'LOG_AGGREGATOR_MAX_DISK_USAGE_PATH', '/var/lib/eda').rstrip('/')
...
def reconfigure_rsyslog(restart_rsyslogd=True): 
    ...
    with tempfile.TemporaryDirectory(dir=config_dir, prefix='rsyslog-conf-') as temp_dir:
