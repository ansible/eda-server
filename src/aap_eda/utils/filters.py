...
class ExternalLoggerEnabled(Filter):
    ...
    def __init__(self, **kwargs):
        ...
        for field_name, field_value in kwargs.items():
            if not isinstance(
                ExternalLoggerEnabled.__dict__.get(field_name, None),
                FieldFromSettings,
            ):
...
class SmartFilter(object):
            ...
            if k == "search":
                ...
                try:
                    ...
                    raise ParseException(
                        "No related field named %s" % relation
                    )
    ...
    def query_from_string(cls, filter_string):
        ...
        * handle key with __ in it.
        ...
        raise RuntimeError(
            "Parsing the filter_string %s went terribly wrong" % filter_string
        )
