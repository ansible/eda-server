...
# EDA
from aap_eda.core.exceptions import PostRunError
...
class RSysLogHandler(logging.handlers.SysLogHandler):
    ...
    def handleError(self, record):
        ...
        try:
            ...
            msg += exc
        msg = "\n".join(
            [msg, force_str(record.msg), ""]
        )  # force_str used in case of translated strings
...
class SpecialInventoryHandler(logging.Handler):
    """Logging handler used for the saving-to-database part of inventory.

    updates ran by the task system
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.skip_level = [logging.WARNING, logging.INFO, logging.DEBUG, 0][
            verbosity
        ]
    ...
    def emit(self, record):
        ...
        if (this_time - self.last_check).total_seconds() > 0.1:
            ...
            if self.cancel_callback():
                raise PostRunError(
                    "Inventory update has been canceled", status="canceled"
                )
        if self.job_timeout and (
            (this_time - self.job_start).total_seconds() > self.job_timeout
        ):
            raise PostRunError(
                "Inventory update has timed out", status="canceled"
            )
    ...
    try:
        ...
        class ColorHandler(ColorizingStreamHandler):
            def colorize(self, line, record):
                ...
                if record.name == "eda.analytics.job_lifecycle":
...
class OTLPHandler(LoggingHandler):
    def __init__(
        ...
        if auth == "basic" and (username is None or password is None):
            raise ValueError(
                "auth type basic requires username and passsword parameters"
            )
        ...
        self.service_name = service_name or (
            sys.argv[1]
            if len(sys.argv) > 1
            else (sys.argv[0] or "unknown_service")
        )
        ...
        if protocol == "grpc":
            otlp_exporter = OTLPGrpcLogExporter(
                endpoint=self.endpoint, insecure=True, headers=headers
            )
        elif protocol == "http":
